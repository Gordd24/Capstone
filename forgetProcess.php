    ...
    if(isset($_POST['account'])&&$_POST['account']=='patient'){
        ...
        echo 'patient';
            ...
            if($stmt->num_rows>0){
                ...
                $output.='<p><a href="http://localhost/capstoneprojeks/reset_password.php?key=' . $key . '&email=' . $email . '&categ='. $categ . '&action=reset" target="_blank">http://localhost/capstoneprojek2/reset_password.php?key=' . $key . '&email=' . $email . '&categ='. $categ . '&action=reset</a></p>';
                $body = $output;
