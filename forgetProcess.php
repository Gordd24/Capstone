    ...
    if(isset($_POST['account'])&&$_POST['account']=='patient'){
        ...
        echo 'patient';
            ...
            if($stmt->num_rows>0){
                ...
                $output.='<p><a href="http://localhost/capstoneprojeks/reset_password.php?key=' . $key . '&email=' . $email . '&categ='. $categ . '&action=reset" target="_blank">http://localhost/capstoneprojeks/reset_password.php?key=' . $key . '&email=' . $email . '&categ='. $categ . '&action=reset</a></p>';
                $body = $output;
    ...
    if(isset($_POST['account'])&&$_POST['account']=='personnel'){
        echo 'personnel';
            ...
            if($stmt->num_rows>0){
                ...
                $output.= '<p>Please click the link to reset password</p>';
                $output.='<p><a href="http://localhost/capstoneprojeks/reset_password.php?key=' . $key . '&email=' . $email . '&categ='. $categ . '&action=reset" target="_blank">http://localhost/capstoneprojek/reset_password.php?key=' . $key . '&email=' . $email .'&categ='. $categ . '&action=reset</a></p>';
                $body = $output;
                $subject = "Password Recovery";
                $email_to = $email;
