        ...
        if(isset($_POST['forgot_account'])&&$_POST['forgot_account']=='patient'){
            ...
            //echo 'patient';
                ...
                if($stmt->num_rows>0){
                    ...
                    // $output.='<p><a href="http://localhost/capstoneprojeks/reset_password.php?key=' . $key . '&email=' . $email . '&categ='. $categ . '&action=reset" target="_blank">localhost/reset_password.php?key=' . $key . '&email=' . $email . '&categ='. $categ . '&action=reset</a></p>';
                    $body = $output;
